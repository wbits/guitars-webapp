    ...
    <section className="space-y-6">
      <header>
        ...
        <p className="text-sm text-slate-600">
          {cognito
            ? 'Manage your Cognito session and API access.'
      ...
      {cognito ? (
        <div className="rounded-md border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-base font-semibold text-slate-900">Cognito sign-in</h2>
          <p className="mt-1 text-sm text-slate-600">
            {current
              ? `Signed in. Access token: ${mask(current)}`
              : 'Not signed in. Use the sign-in page to obtain a JWT.'}
          </p>
        ...
        <>
          <div className="rounded-md border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
            ...
            <p className="mt-1">
              ...
              <code className="rounded bg-amber-100 px-1">{RUNTIME_TOKEN_KEY}</code>) and take
              precedence over any token baked into the build at build time. Anything baked into the
              build is publicly visible in the static bundle.
            </p>
          </div>
          ...
          >
            ...
            />
            <p className="help">
              Stored only in this tab's <code>sessionStorage</code>. Closing the tab discards it.
            </p>
